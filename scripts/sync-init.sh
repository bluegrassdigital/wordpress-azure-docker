...
# ---- Optional: audit and sanitize legacy .htaccess directives that break under PHP-FPM ----
# Set HTACCESS_SANITIZE=1 to automatically comment out offending lines (non-destructive backup created).
audit_htaccess() {
  local base="$1"; shift || true
  local audit_log="$LOG_BASE_DIR/htaccess-audit.log"
  mkdir -p "$(dirname "$audit_log")" || true
  echo "$(date) Auditing .htaccess under $base" | tee -a "$audit_log"
  local patterns='^\s*(php_value|php_flag|AddHandler|AddType\s+application/x-httpd-php|suPHP_ConfigPath|<IfModule\s+mod_php)'
  local files
  IFS=$'\n' read -r -d '' -a files < <(find "$base" -type f -name '.htaccess' -print0 | xargs -0 -I{} echo {} && printf '\0') || true
  for f in "${files[@]}"; do
    if [[ -f "$f" ]]; then
      if grep -nE "$patterns" "$f" >/dev/null 2>&1; then
        echo "$(date) Found legacy directives in: $f" | tee -a "$audit_log"
        grep -nE "$patterns" "$f" | sed 's/^/    /' | tee -a "$audit_log" || true
        if [[ "${HTACCESS_SANITIZE:-0}" == "1" ]]; then
          cp "$f" "$f.bak" || true
          # Comment out known-incompatible lines
          sed -ri "s#^\s*(php_value|php_flag|AddHandler|AddType\s+application/x-httpd-php|suPHP_ConfigPath)#\# sanitized by sync-init: \0#I" "$f" || true
          # Extract php_value/php_flag entries and materialize them into a per-dir PHP config
          local user_ini_dir
          user_ini_dir="$(dirname "$f")"
          local user_ini="$user_ini_dir/.user.ini"
          {
            echo "; generated by sync-init (from $f) on $(date)"
            # php_value key value  -> key = value
            # php_flag  key on|off -> key = On|Off
            awk 'BEGIN{IGNORECASE=1}
                 /^\s*php_value[\t ]+/ {k=$2; $1=""; $2=""; sub(/^\s+/, ""); printf("%s = %s\n", k, $0)}
                 /^\s*php_flag[\t ]+/  {k=$2; v=tolower($3); if(v=="on"){v="On"} else if(v=="off"){v="Off"}; printf("%s = %s\n", k, v)}' "$f.bak" 2>/dev/null
          } >> "$user_ini" || true
          chown www-data:www-data "$user_ini" 2>/dev/null || true
          echo "$(date) Wrote PHP overrides to: $user_ini" | tee -a "$audit_log"
          echo "$(date) Sanitized legacy directives in: $f (backup: $f.bak)" | tee -a "$audit_log"
        fi
      fi
    fi
  done
...
# Audit/sanitize .htaccess in the served path (home-only mode)
audit_htaccess "/home/site/wwwroot"
...
# Audit/sanitize .htaccess in both trees prior to switching
audit_htaccess "/home/site/wwwroot"
audit_htaccess "/homelive/site/wwwroot"
